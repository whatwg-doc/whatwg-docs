...
(function(window,KISSY){

    KISSY.add('table/template',function(S){
...
(function (window, KISSY) {
    ...
    var TableHelper = {
        allUpdate: function(){
            //配置文件.
            var chartsArr = $('.wdm_table')
                ;

            var container,//表格填充容器.
                callback,
                tTmpl,   //表格模板
                ajaxUrl,
                hidden_param,
            ...
            for(var i = 0,len = chartsArr.length; i < len; i++){
                callback = $(chartsArr[i]).attr('data-callback');
                tTmpl = $(chartsArr[i]).attr('data-for');
                container = $('#' + tTmpl).attr('data-for');
                ajaxUrl = $(chartsArr[i]).attr('data-uri');
                hidden_param = $(chartsArr[i]).children('.hidden_param');

                for(var j = 0,size = hidden_param.length; j < size; j++){
                    dataObj[hidden_param[j].getAttribute('data-name')] = hidden_param[j].getAttribute('data-value');
                }
            ...
            KISSY.ajax({
                ...
                success:function (result) {
                    try {
                        ...
                        KISSY.use('table/template',function(S,tmpl){
                            var html = tmpl.render(options.tTmpl, result.table);
        ...
        oneUpdate: function(tmpl){
            ...
            for(var i = 0,len = chartsArr.length; i < len; i++){
                ...
                if($(chartsArr[i]).attr('data-for').indexOf(tmpl) > -1){
                    callback = $(chartsArr[i]).attr('data-callback');
                    tTmpl = $(chartsArr[i]).attr('data-for');
                    container = $('#' + tTmpl).attr('data-for');
                    ajaxUrl = $(chartsArr[i]).attr('data-uri');
    ...
    TableHelper.allUpdate();

    //外部触发.
    KISSY.use('event',function(S,Event){
        Event.delegate(document,'click','.table_ctl',function(e){
            var $target = $(e.currentTarget),
                tmpl = $target.attr('data-for'),
                container = $('#' + tmpl).attr('data-for')
                ;

            $('#' + container).html('');
            TableHelper.oneUpdate(tmpl);
        });
    });
...
//        };
//        var html = tmpl.render('test', data);
